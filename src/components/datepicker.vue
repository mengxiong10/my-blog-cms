...
<div class="mz-datepicker" :style="{width:width+'px'}">
    ...
    <a v-if="clearable&&text" @click.stop="clear"></a> <i @click="click"></i>
    <div class="mz-datepicker-popup" :class="{'mz-datepicker-popup-left':left}" v-show="show"  transition="mz-datepicker-popup" tabindex="-1" @blur="show = false" @mousedown="$event.preventDefault()" @keyup.up="changeMonth(-1,1)" @keyup.down="changeMonth(1,1)" @keyup.left="changeYear(-1,1)" @keyup.right="changeYear(1,1)" ref="popup">
        <div class="mz-calendar-top" v-if="range&&!en">
            <template v-for="(item,index) in ranges"> <i v-if="index"></i>
            ...
            <template v-for="no in count">
                <div class="mz-calendar">
                    <div class="mz-calendar-header">
                        ...
                        <a v-if="!en" class="mz-calendar-year-select" :title="selectYearTitle" @click="showYear(no+1)">{{selectNo(no+1,'now').getFullYear()+(en?"":"年")}}</a>
                        <a v-if="!en" class="mz-calendar-month-select" :title="selectMonthTitle" @click="showMonth(no+1)">{{months[selectNo(no+1,'now').getMonth()]}}</a>
                        <a v-if="en" class="mz-calendar-month-select" :title="selectMonthTitle" @click="showMonth(no+1)">{{months[selectNo(no+1,'now').getMonth()]}}</a>
                        <a v-if="en" class="mz-calendar-year-select" :title="selectYearTitle" @click="showYear(no+1)">{{selectNo(no+1,'now').getFullYear()+(en?"":"年")}}</a>
                    ...
                    <table cellspacing="0" cellpadding="0">
                        ...
                        <tr v-if="selectNo(no+1,'date')" v-for="i in 6">
                            <td v-for="j in 7" v-text="selectNo(no+1,'date')[(i-1) * 7 + (j-1)].text" :title="selectNo(no+1,'date')[(i-1) * 7 + (j-1)].title" :class="selectNo(no+1,'date')[(i-1) * 7 + (j-1)].status" @click="select(selectNo(no+1,'date')[(i-1) * 7 + (j-1)], no+1)"></td>
                        </tr>
                    </table>
                    <div class="mz-calendar-year-panel" transition="mz-calendar-panel" v-if="selectNo(no+1,'showYear')">
                        <a class="mz-calendar-year-panel-prev" @click="changeYearRange(no+1,-1)"></a>
                        <a class="mz-calendar-year-panel-year" v-for="item in selectNo(no+1,'years')" :class="item.status" @click="selectYear(item,no+1)">{{item.year+(en?"":"年")}}</a>
                        <a class="mz-calendar-year-panel-next" @click="changeYearRange(no+1,1)"></a>
                    </div>
                    <div class="mz-calendar-month-panel" transition="mz-calendar-panel" v-if="selectNo(no+1,'showMonth')">
                        <a v-for="item in selectNo(no+1,'months')" class="mz-calendar-month-panel-month" :class="item.status" @click="selectMonth(item,no+1)">{{months[item.month-1].substr(0,3)}}</a>
                    </div>
                </div>
 ...
 export default {
    ...
    computed: {
        ...
        startTime:{
            ...
            set(val){
                this.$emit('input',[val,this.entTime])
            }
        },
        endTime:{
            get () {
