<template>
	<div class="mz-datepicker" :style="{width:width+'px'}">
		<input :value="text" readonly :disabled="disabled" :placeholder="place" :class="{focus:show}" @click="click" @mousedown="$event.preventDefault()"
		/>
		<a v-if="clearable&&text" @click.stop="clear"></a>
		<i @click="click"></i>
		<div class="mz-datepicker-popup" :class="{'mz-datepicker-popup-left':left}" v-show="show" transition="mz-datepicker-popup"
			tabindex="-1" @blur="show = false" @mousedown="$event.preventDefault()" @keyup.up="changeMonth(-1,1)" @keyup.down="changeMonth(1,1)"
			@keyup.left="changeYear(-1,1)" @keyup.right="changeYear(1,1)" ref="popup">
				...
				<template v-for="no in count">
					<div class="mz-calendar">
						<div class="mz-calendar-header">
							...
							<a class="mz-calendar-year-select" :title="selectYearTitle" @click="showYear(no)">{{selectNo(no,'now').getFullYear()+ '年'}}</a>
							<a class="mz-calendar-month-select" :title="selectMonthTitle" @click="showMonth(no)">{{months[selectNo(no,'now').getMonth()]}}</a>
						...
						<table cellspacing="0" cellpadding="0">
							...
							<tr v-if="date[no]" v-for="i in 6">
								<td v-for="j in 7" v-text="date[no][(i-1) * 7 + (j-1)].text" :title="date[no][(i-1) * 7 + (j-1)].title"
									:class="date[no][(i-1) * 7 + (j-1)].status" @click="select(date[no][(i-1) * 7 + (j-1)], no)"></td>
							</tr>
						</table>
						<div class="mz-calendar-year-panel" transition="mz-calendar-panel" v-if="selectNo(no,'showYear')">
							<a class="mz-calendar-year-panel-prev" @click="changeYearRange(no,-1)"></a>
							<a class="mz-calendar-year-panel-year" v-for="item in selectNo(no,'years')" :class="item.status" @click="selectYear(item,no)">{{item.year+(en?"":"年")}}</a>
							<a class="mz-calendar-year-panel-next" @click="changeYearRange(no,1)"></a>
						</div>
						<div class="mz-calendar-month-panel" transition="mz-calendar-panel" v-if="selectNo(no,'showMonth')">
							<a v-for="item in selectNo(no,'months')" class="mz-calendar-month-panel-month" :class="item.status" @click="selectMonth(item,no)">{{months[item.month-1].substr(0,3)}}</a>
						</div>
					</div>
...
<script>
 export default {
        ...
        text: function() {
            if (this.range) {
                if (this.startTime && this.endTime) {
                    return this.stringify(this.parse(this.startTime, false),'yyyy-MM-dd') + ' ~ ' + this.stringify(this.parse(this.endTime, false),'yyyy-MM-dd');
                } else {
                    return '';
                }
            } else {
                return this.stringify(this.parse(this.time, false),'yyyy-MM-dd');
            }
        }
    },
    watch: {
    ...
    methods: {
        ...
        clear: function() {
            var self = this;
            self.time1 = self.time2 = self.startTime = self.endTime = self.time = '';
            self.now1 = new Date();
            self.now2 = new Date();
            if (self.range && self.confirm) {
                self.onConfirm && self.onConfirm(self.startTime, self.endTime);
