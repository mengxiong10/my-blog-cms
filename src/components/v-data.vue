...
<div class="mz-datepicker" :style="{width:width+'px'}">
    <input :value="value" readonly :disabled="disabled" :placeholder="place" :class="{focus:show}" @click="click" @mousedown="$event.preventDefault()"/>
    <a v-if="clearable&&value" @click.stop="clear"></a> <i @click="click"></i>
    <div class="mz-datepicker-popup" :class="{'mz-datepicker-popup-left':left}" v-show="show"  transition="mz-datepicker-popup" tabindex="-1" @blur="show = false" @mousedown="$event.preventDefault()" @keyup.up="changeMonth(-1,1)" @keyup.down="changeMonth(1,1)" @keyup.left="changeYear(-1,1)" @keyup.right="changeYear(1,1)" v-el:popup>
            ...
            <template v-for="no in count">
                <div class="mz-calendar">
                    ...
                    <div class="mz-calendar-year-panel" transition="mz-calendar-panel" v-if="this['showYear'+(no+1)]">
                        ...
                        <a class="mz-calendar-year-panel-year" v-for="item in this['years'+(no+1)]" :class="item.status" @click="selectYear(item,no+1)">{{item.year+(en?"":"年")}}</a>
